...
class HomeScreen(Screen):
    ...
    def save_image(self, instance):
        filechooser = FileChooserIconView(select_string="Save")
        filechooser.bind(on_submit=lambda _, selection: self.do_save_image(selection))
        self.add_widget(filechooser)

    def do_save_image(self, selection):
        if selection:
            cv2.imwrite(selection[0], self.app.processed_image)
            self.remove_widget(self.children[0])  # Remove filechooser
...
class CropScreen(Screen):
    def __init__(self, app, **kwargs):
        ...
        # Interactive Crop Tool Placeholder
        crop_label = Label(text="Drag to crop the image.")
        self.layout.add_widget(crop_label)
