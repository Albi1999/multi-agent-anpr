...
class ImageProcessor:
    ...
    def select_corners(self, image, selected_corners):
        """Applies perspective correction using selected points from Dash."""
        if len(selected_corners) != 4:
            return image  # Return original image if not enough points are selected
    ...
    def perspective_correction(self, image, selected_corners):
        """Applies perspective correction using selected points from Dash."""
        if len(selected_corners) != 4:
            return image  # Return original image if not enough points are selected
    ...
    def paint_image(self, image, drawing_overlay):
        """Applies a drawing overlay (from DashCanvas) onto the image."""
        if drawing_overlay is None:
            return image  # Return original image if no overlay data is provided

        # Combine original image with the overlay
        result = cv2.addWeighted(image, 1, drawing_overlay, 0.5, 0)
        return result

    def character_segmentation(self, image):
        """Segments characters and returns a list of character images."""
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
def parse_json_to_cv(json_data, image_shape):
    """
    Converts DashCanvas JSON drawing data into an OpenCV-compatible mask.
    """
    import json
    ...
    # Parse the JSON data
    paths = json.loads(json_data)["objects"]
        ...
        if path["type"] == "path":
            ...
            if len(points) > 1:
                for i in range(len(points) - 1):
                    x1, y1 = int(points[i][1]), int(points[i][2])
                    x2, y2 = int(points[i + 1][1]), int(points[i + 1][2])
                    cv2.line(overlay, (x1, y1), (x2, y2), (255, 0, 0), thickness=2)
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Button("Apply Painting", id="apply-painting-btn", n_clicks=0, style={
                            "width": "100%", "padding": "10px", "backgroundColor": "#dc3545", "color": "#fff",
                            "border": "none", "borderRadius": "5px", "marginTop": "10px"
                        }),

                        # Graph for corner selection
                        dcc.Graph(id="image-graph", config={"scrollZoom": False, "displayModeBar": False}),

                        # Display segmented characters
                        html.Div(id="segmented-characters", style={"display": "flex", "flexWrap": "wrap", "marginTop": "20px"}),
...
def process_image(
    ...
    if not ctx.triggered:
        return (
            None, blur_style, exposure_style, contrast_style, brightness_style, threshold_style, ""
        )
    ...
    if processed_image is not None:
        if trigger_id == 'auto-crop-btn':
            ...
            processed_image = image_processor.select_corners(processed_image)
        elif trigger_id == 'perspective-correction-btn':
            processed_image = image_processor.perspective_correction(processed_image)
        elif trigger_id == 'paint-image-btn':
            processed_image = image_processor.paint_image(processed_image)
        elif trigger_id == 'segment-characters-btn':
            characters = image_processor.character_segmentation(processed_image)
...
# Handle selecting corners inside Dash
@app.callback(
    Output('image-display', 'children'),
    Input('image-graph', 'clickData'),
    State('stored-image-path', 'data')
)
def select_corners(click_data, image_path):
    """Selects four corners for perspective correction using user clicks in Dash."""
    if not click_data or not image_path:
        return dash.no_update

    global selected_corners
    selected_corners.append((click_data['points'][0]['x'], click_data['points'][0]['y']))

    if len(selected_corners) == 4:
        image = cv2.imread(image_path)
        processed_image = image_processor.perspective_correction(image, selected_corners)
        encoded_image = image_to_base64(processed_image, format="png")
        selected_corners = []
...
# Handle painting inside Dash
@app.callback(
    Output("image-display", "children"),
    Input("apply-painting-btn", "n_clicks"),
    State("paint-canvas", "json_data"),
    State("stored-image-path", "data")
)
def update_painted_image(n_clicks, json_data, image_path):
    """Overlay user drawings from DashCanvas onto the image."""
    if n_clicks == 0 or json_data is None or image_path is None:
        return dash.no_update

    # Load the original image
    image = cv2.imread(image_path)
    
    # Convert DashCanvas JSON data into an OpenCV mask
    overlay = parse_json_to_cv(json_data, image.shape[:2])
    
    # Blend the drawing with the original image
    result = cv2.addWeighted(image, 1, overlay, 0.5, 0)

    # Convert result to Base64 for Dash display
    encoded_image = image_to_base64(result, format="png")
    
    return html.Img(src=encoded_image, style={"width": "90%", "borderRadius": "10px"})


# Handle character segmentation
@app.callback(
    Output("segmented-characters", "children"),
    Input("segment-characters-btn", "n_clicks"),
    State("stored-image-path", "data")
)
def segment_characters(n_clicks, image_path):
    """Displays segmented characters in Dash instead of OpenCV pop-ups."""
    if n_clicks == 0 or image_path is None:
        return dash.no_update

    image = cv2.imread(image_path)
    characters = image_processor.character_segmentation(image)

    if not characters:
        return html.Div("No characters found.", style={"color": "red"})

    images = [html.Img(src=image_to_base64(char, "png"), style={"margin": "5px"}) for char in characters]
    return html.Div(images, style={"display": "flex", "flexWrap": "wrap"})
