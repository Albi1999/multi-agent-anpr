...
def process_image(
    ...
    suggest_tool_clicks, select_corners_clicks, perspective_correction_clicks,
    segment_characters_clicks, adaptive_dataset_clicks,
    ...
    brightness_value, brightness_style, threshold_block, threshold_style, current_image_path
):
    ...
    if trigger_id == 'upload-image':
        if contents:
            ...
            cv2.imwrite(upload_path, uploaded_image)
    ...
    # Apply transformations and AI suggestions
    if processed_image is not None:
        img = cv2.imread(current_image_path)

        if trigger_id == 'auto-crop-btn':
            processed_image = auto_crop_license_plate(img)
        elif trigger_id == 'blur-btn':
            blur_kernel = max(1, blur_kernel // 2 * 2 + 1)  # Ensure odd kernel size
            processed_image = cv2.GaussianBlur(img, (blur_kernel, blur_kernel), 0)
        elif trigger_id == 'exposure-btn':
            processed_image = adjust_exposure(img, exposure_value)
        elif trigger_id == 'contrast-btn':
            processed_image = adjust_contrast(img, contrast_value)
        elif trigger_id == 'brightness-btn':
            processed_image = adjust_brightness(img, brightness_value)
        elif trigger_id == 'threshold-btn':
            processed_image = adaptive_thresholding(img, threshold_block, 2)
        ...
        elif trigger_id == 'suggest-tool-btn':
            suggestion_output = suggest_tool(current_image_path)
        elif trigger_id == 'select-corners-btn':
            select_corners(current_image_path)
        elif trigger_id == 'perspective-correction-btn':
            processed_image = perspective_correction(image_path=current_image_path)
        elif trigger_id == 'segment-characters-btn':
            characters = character_segmentation(current_image_path)
            segment_dir = "processed/characters/"
            os.makedirs(segment_dir, exist_ok=True)
            for i, char_img in enumerate(characters):
                cv2.imwrite(f"{segment_dir}/char_{i}.png", char_img)
        elif trigger_id == 'adaptive-dataset-btn':
    ...
    return dash.no_update, blur_style, exposure_style, contrast_style, brightness_style, threshold_style, "", dash.no_update
