...
class ImageProcessor:
    ...
    def select_corners(self, image):
        """Simulate selecting corners by clicking on an image in a pop-up window."""
        ...
        while True:
            ...
            elif key == ord('f') and len(self.selected_points) == 4:
                cv2.destroyAllWindows()
                return np.float32(self.selected_points)
    ...
    def perspective_correction(self, image):
        """Corrects the perspective of an image using selected points."""
        src = self.select_corners(image)
        if src is None:
            return image
    ...
    def paint_image(self, image):
        """Allows users to draw on an image interactively."""
        drawing = False
        last_point = None
        img_display = image.copy()
    ...
    def character_segmentation(self, image, deviation=10):
