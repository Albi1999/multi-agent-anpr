...
# Layout of the app
app.layout = html.Div([
    html.H1("Interactive Image Processing App", style={"textAlign": "center"}),

    # File upload component
    dcc.Upload(
        id='upload-image',
        children=html.Button('Upload Image', style={"fontSize": "16px"}),
        multiple=False
    ),
    ...
    html.Div([
        ...
        html.Button('Crop & Resize', id='crop-btn', n_clicks=0, style={"margin": "10px"}),

        dcc.Slider(id='blur-slider', min=1, max=21, step=2, value=5, 
                   marks={i: str(i) for i in range(1, 22, 2)}, tooltip={"placement": "bottom"}),
        html.Button('Blur', id='blur-btn', n_clicks=0, style={"margin": "10px"}),
        ...
        dcc.Slider(id='threshold2-slider', min=0, max=255, step=1, value=150, 
                   marks={i: str(i) for i in range(0, 256, 50)}, tooltip={"placement": "bottom"}),
        html.Button('Edge Detection', id='edge-btn', n_clicks=0, style={"margin": "10px"}),

        html.Button('Grayscale', id='grayscale-btn', n_clicks=0, style={"margin": "10px"}),
        html.Button('Sharpen', id='sharpen-btn', n_clicks=0, style={"margin": "10px"}),
        html.Button('Invert Colors', id='invert-btn', n_clicks=0, style={"margin": "10px"}),
    ], style={"textAlign": "center", "marginTop": "20px"}),

    # Download button
    html.A("Download Processed Image", id="download-link", download="processed_image.jpg", 
           href="", target="_blank", style={"display": "block", "textAlign": "center", "marginTop": "20px"})
])
